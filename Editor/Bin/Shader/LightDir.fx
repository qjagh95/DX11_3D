...
PS_OUTPUT_LIGHTACC LightAccPS(VS_OUTPUT_UV input)
{
    ...
    float3 vNormal = g_GBufferNormalTex.Sample(g_GBufferSampler, input.vUV).xyz;
    ...
    float3 Result = WVPPos;
    Result *= vDepth.a;

    //NDC를 가지고 HCS(WV변환좌표 View를 기준으로 하고있음)를 구한다.
    float3 ViewHCS = mul(float4(Result, 1.0f), g_InvProjection).xyz;
    float3 ToCamera = normalize(-input.vPos).xyz;
    ...
    if (g_Light.LightType == LIGHT_DIRECTION)
        ...
        ComputePointLight(vNormal, ViewHCS, ToCamera, Ambient, Diffuse, Specluar);
    else if (g_Light.LightType == LIGHT_SPOT)
        ComputeSpotLight(vNormal, ViewHCS, ToCamera, Ambient, Diffuse, Specluar);
    else if (g_Light.LightType == LIGHT_SPOT_BOMI)
        ComputeSpotBomiLight(vNormal, ViewHCS, ToCamera, Ambient, Diffuse, Specluar);
